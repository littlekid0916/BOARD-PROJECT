...
import { BOARD_UPDATE_RATH, COUNT_BY_PAGE_COMMENT, MAIN_PATH, USER_PAGE_PATH } from 'src/constants';
import './style.css';
...
export default function BoardDetail() {
  ...
  const Board = () => {
    ...
    const onUpdateButtonClickHandler = () => {
      if (!board) return;
      navigator(BOARD_UPDATE_RATH(board?.boardNumber));
    }
  ...
  const FavoriteList = () => {
    ...
    return (
      <div className='favorite-list-box'>
        <div className='favorite-list-title'>좋아요 <span className='favorite-list-emphasis'>{favoriteList.length}</span></div>
    ...
    return (
      <div className='comment-list-box'>
        <div className='comment-list-top'>
          <div className='comment-list-title'>댓글 <span className='comment-list-title-emphasts'>{commentList.length}</span></div>
  ...
  //        effect        //
  // description: 게시물 번호가 바뀔때 마다 새로운 정보 받아오기 //
  useEffect(() => {
