...
export default function Authentication() {
  ...
  const SignUpCard = () => {
    ...
    const checkPage2 = () => {
      ...
      // description: 백엔드로 데이터 전송 (회원가이 포맷에 맞춰서) //
          ...
          <div className='auth-card-top-input-container'>
            {
              page === 1 ? (
                <>
                  <InputBox label='이메일 주소*' type='text' placeholder='이메일 주소를 입력해주세요.' error={emailPatternError || emailDuplicationError} helper={emailPatternError ? '이메일 주소 포멧이 맞지 않습니다.' : emailDuplicationError ? '중복되는 이메일 주소입니다.' : ''} value={email} setValue={setEmail} />
